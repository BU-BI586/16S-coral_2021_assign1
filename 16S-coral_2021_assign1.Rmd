...
We chose the Silva database because it contains a comprehensive library of 16S sequences and is specifically formatted for Dada2.
...
We next generate a csv file for the taxonomy so that it's easier to map the sequences for the heatmap.
```{r, eval=FALSE}
...
Now, we make bar plots representing the abundances of the 90 most abundant taxa by sample.
```{r}
...
p <-plot_bar(ps.top90, x= "Sample",fill="Class") 
p <- p + scale_x_discrete(labels=c('Diseased1', 'Healthy1', "Healthy2", "Diseased2", "Water column"))
p
```
...
#
```
Now, we make a .csv file for the phyloseq data. This contains the abundances of each individual in each sample by Class. We use the psmelt function prior to writing the .csv because it will allow us to create graphics in ggplot with the data. 
```{r, echo=FALSE}
...
p3 <- ggplot(psz, aes(x=Sample, y=Abundance, fill=Class))
